...
def write_csv(records, path=os.path.join(DATA, "places.csv")):
...
def clean(v):
    """Collapse whitespace (incl. newlines) and neutralise quotes/backslashes so
    the value is safe inside a one-line JS string literal."""
    v = re.sub(r"\s+", " ", (v or "")).strip()
    return v.replace("\\", "").replace('"', "'")


def video_url(vid):
    return ("https://www.youtube.com/watch?v=" + vid) if vid else ""


def load_existing_entries(path):
    """Ordered list of the PLACES entries already in a <slug>.js, as string fields."""
    out = []
    ...
    for m in re.finditer(r"\{[^{}]*\}", text):
        blob = m.group(0)
        tm = re.search(r'title:\s*"((?:[^"\\]|\\.)*)"', blob)
        if not tm:
            continue
        lm = re.search(r'location:\s*"((?:[^"\\]|\\.)*)"', blob)
        vm = re.search(r'video:\s*"([^"]*)"', blob)
        la = re.search(r"lat:\s*(null|-?\d+(?:\.\d+)?)", blob)
        ln = re.search(r"lng:\s*(null|-?\d+(?:\.\d+)?)", blob)
        out.append({
            "title": tm.group(1),
            "location": lm.group(1) if lm else "",
            "video": vm.group(1) if vm else "",
            "lat": la.group(1) if la else "null",
            "lng": ln.group(1) if ln else "null",
        })
    return out


def write_js(records, path, map_title):
    """Non-destructive merge. Existing entries keep their title, coordinates and
    order; we only add a `location` where it's missing and fill null coordinates
    when we resolved one. Genuinely new shops (not already in the file) are
    appended. Hand-edited titles are never overwritten."""
    safe_title = map_title.replace('"', "'")
    existing = load_existing_entries(path)

    rec_by_key = {}
    for r in records:
        key = (video_url(r.get("video_id") or ""), clean(r.get("name")))
        rec_by_key.setdefault(key, r)

    merged, seen, kept, enriched = [], set(), 0, 0
    for e in existing:                           # 1) preserve + enrich existing
        key = (e["video"], e["title"])
        seen.add(key)
        r = rec_by_key.get(key)
        location = e["location"] or (clean(r.get("address")) if r else "")
        if location and not e["location"]:
            enriched += 1
        lat, lng = e["lat"], e["lng"]
        if (lat == "null" or lng == "null") and r and r.get("lat") is not None:
            lat, lng = str(r["lat"]), str(r["lng"])
        else:
            kept += 1 if lat != "null" else 0
        merged.append({"title": e["title"], "location": location,
                       "video": e["video"], "lat": lat, "lng": lng})

    added = 0
    for r in records:                            # 2) append genuinely new shops
        name = clean(r.get("name"))
        key = (video_url(r.get("video_id") or ""), name)
        if key in seen:
            continue
        # Skip obvious parse failures (a real shop name isn't a paragraph or a URL).
        if len(name) > 60 or "http" in name.lower():
            continue
        seen.add(key)
        added += 1
        merged.append({
            "title": clean(r.get("name")),
            "location": clean(r.get("address")),
            "video": key[0],
            "lat": str(r["lat"]) if r.get("lat") is not None else "null",
            "lng": str(r["lng"]) if r.get("lng") is not None else "null",
        })

    with open(path, "w", encoding="utf-8") as f:
        f.write("// Generated by extract_places.py (non-destructive merge). Loaded before map.js.\n")
        f.write("// Existing titles/coordinates are preserved; only location + null coords are filled.\n\n")
        f.write('const MAP_TITLE = "%s";\n\n' % safe_title)
        f.write("const PLACES = [\n")
        for e in merged:
            f.write('  { title: "%s", location: "%s", video: "%s", lat: %s, lng: %s },\n'
                    % (e["title"], e["location"], e["video"], e["lat"], e["lng"]))
        f.write("];\n")

    if existing:
        print("  (merged: kept %d entries, added %d location(s), %d new shop(s))"
              % (len(existing), enriched, added))
    return path
